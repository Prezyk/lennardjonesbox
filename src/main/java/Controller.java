...
public class Controller {
    ...
    XYChart.Series eKin;
    XYChart.Series ePot;
    XYChart.Series eElat;
    XYChart.Series eTotal;
    ...
    public void initialize() {
        sliderMolecules.valueProperty().addListener(l -> {
            txtMolecules.setText(String.valueOf(sliderMolecules.getValue()));
        });
    ...
    public void btnLoadAction() throws IOException {
        ...
        for (int i = 0; i < molec.getN(); i++) {
            for (int a = 0; a < molec.getMoleculesQuantity(); a++) {
                ...
                if (i == 0) {
                    pathList.get(a).getElements().add(new MoveTo(xCoord, yCoord));
                } else {
            ...
            eKin.getData().add(new XYChart.Data(molec.getTime()[i], molec.getEkin()[i]));
            ePot.getData().add(new XYChart.Data(molec.getTime()[i], molec.getEpot()[i]));
            eElat.getData().add(new XYChart.Data(molec.getTime()[i], molec.getElastE()[i]));
            eTotal.getData().add(new XYChart.Data(molec.getTime()[i], molec.getEpot()[i] + molec.getEkin()[i] + molec.getElastE()[i]));
        ...
        for (int i = 0; i < molec.getMoleculesQuantity(); i++) {
            ...
            ptr.get(i).setDuration(Duration.seconds(molec.getTime()[molec.getTime().length - 1]));
            ptr.get(i).setPath(pathList.get(i));
            ptr.get(i).setNode(atoms.get(i));
        }
    ...
    new Thread(){
        ...
        public synchronized void start() {
            ...
            for(int i=0; i<n; i++) {
                ...
                ePot.getData().add(new XYChart.Data(currentTime, md.getPotE()));
                eElat.getData().add(new XYChart.Data(currentTime, md.getElastE()));
                eTotal.getData().add(new XYChart.Data(currentTime, md.getPotE()+md.getKinE()+md.getElastE()));
            ...
            for(int i=0; i<molecules; i++) {
                ptr.add(new PathTransition());
                ptr.get(ptr.size()-1).setDuration(Duration.seconds(threadTime));
                ptr.get(ptr.size()-1).setPath(pathList.get(i));
                ptr.get(ptr.size()-1).setNode(atoms.get(i));
            }
            calculationStatusLabel.setText("Calculation done");
            tbtnAnim.setDisable(false);
            tbtnChart.setDisable(false);
